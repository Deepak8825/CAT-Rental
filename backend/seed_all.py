...
def seed_all():
    ...
    # Drop and recreate all tables for a fresh clean state
    ...
    try:
        ...
        print("5. Seeding Rentals...")
        rent_df = pd.read_csv(os.path.join(data_dir, "rentals.csv"))
        valid_rentals = rent_df[rent_df["equipment_id"].isin(valid_equip_ids)].head(2000)
        valid_rental_ids = set(valid_rentals["id"])
        for _, row in valid_rentals.iterrows():
            session.add(Rental(
                id=parse_uuid(row["id"]),
        ...
        session.commit()
        print(f"  - {len(valid_rentals)} rentals seeded.")

        print("6. Seeding Daily Logs...")
        ...
        for _, row in matching_logs.iterrows():
            session.add(DailyLog(
                id=parse_uuid(row["id"]),
                equipment_id=parse_uuid(row["equipment_id"]),
                rental_id=parse_uuid(row.get("rental_id")) if row.get("rental_id") in valid_rental_ids else None,
        ...
        print("7. Seeding Sensor Readings...")
        ...
        print("8. Seeding Maintenance Records...")
        ...
        print("9. Seeding Events...")
        event_df = pd.read_csv(os.path.join(data_dir, "events.csv"))
        matching_events = event_df[event_df["equipment_id"].isin(valid_equip_ids)].head(1000)
        ...
        session.commit()
        print(f"  - {len(matching_events)} events seeded.")

        print("10. Seeding Feedback...")
        feed_df = pd.read_csv(os.path.join(data_dir, "feedback.csv"))
        sample_feed = feed_df.head(1000)
        for _, row in sample_feed.iterrows():
        ...
        session.commit()
        print(f"  - {len(sample_feed)} feedback records seeded.")

        print("11. Seeding Admin & Customer Auth Accounts...")
        ...
        session.commit()
        print("  - Default accounts (admin01@gmail.com & user01@gmail.com) created.")

        print("\nAll database seed operations completed successfully!")
